...
void set_frame_max_width_impl(tframe_t *frame, int value){
...
typedef void (*method_t)(int);

method_t make_method(
    tframe_t *self,
    void (*impl)(tframe_t *, int))
{
    uint8_t *code = mmap(
        NULL,
        32,
...
void test(tframe_t *frame, int value)
{
    set_frame_max_width_impl(frame, value);
}
...
int init_tframe(tframe_t *frame){
    ...
    frame->max_zz = make_method(frame, set_frame_max_width_impl);
